...
erpnext.resource_calendar.resourceCalendar = class ResourceCalendar {
			...
			eventReceive: function (info) {
				...
				if (target == "AssignTo") {
					...
					if (user.filter(f => f).length) {
						frappe.xcall('hrms.hr.page.resource_planning_view.resource_planning_view.add_to_doc', {
	...
	get_events(info, callback) {
		...
		frappe.xcall("hrms.hr.page.resource_planning_view.resource_planning_view.get_events", {
			start: moment(info.start).format("YYYY-MM-DD"),
			end: moment(info.end).format("YYYY-MM-DD"),
			filters: filters,
			group_by: this.group,
	...
	get_resource_total(info) {
		frappe.xcall("hrms.hr.page.resource_planning_view.resource_planning_view.get_resources_total", {
			start: moment(info.start).format("YYYY-MM-DD"),
			end: moment(info.end).format("YYYY-MM-DD"),
			group_by: this.group,
		}).then(r => {
	...
	add_shift_buttons() {
		frappe.xcall("hrms.hr.page.resource_planning_view.resource_planning_view.get_shift_types")
			.then(res => {
