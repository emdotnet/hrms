...
def get_assignments(start, end, filters=None, group_by=None, return_records=False):
	...
	for d in records:
		...
		if d.docstatus == 0:
			e.update(
				{
					"primary_action": "hrms.hr.page.resource_planning_view.resource_planning_view.submit_shift_assignment",
...
def get_assignment_requests(start, end, filters=None, group_by=None):
	...
	for d in requests:
		...
		e = {
			...
			"designation": d.get("designation"),
			"shift_type": d.get("shift_type"),
			"primary_action": "hrms.hr.page.resource_planning_view.resource_planning_view.approve_shift_request",
			"primary_action_label": _("Approve"),
			"secondary_action": "hrms.hr.page.resource_planning_view.resource_planning_view.reject_shift_request",
